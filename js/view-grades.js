...
function renderViewGrades(container) {
  ...
  gStep1.onsubmit = async (ev) => {
    ...
    } catch(e) { gError.textContent = e.message; }
    gLoading.textContent = '';
  };
  ...
  gStep2.onsubmit = async (ev) => {
    ev.preventDefault();
    gGradesView.classList.add('hidden');
    gError.textContent = ''; gLoading.textContent = "Cargando notas...";
    try {
  ...
  gGradeFilter.oninput = () => {
    ...
    gradesCurrentPage=1; renderGradesTable();
  };
  gPrev.onclick = () => {if(gradesCurrentPage>1){gradesCurrentPage--;renderGradesTable();}};
  gNext.onclick = () => {
    const pages = Math.ceil(gradesFiltered.length/GRD_PAGE_SZ);
    if(gradesCurrentPage<pages){gradesCurrentPage++;renderGradesTable();}
  };
}